...
export function AuthForm({ mode, nextPath, initialError = null, configError = null }: AuthFormProps) {
  ...
  async function bootstrapWorkspaceSession() {
    ...
    throw new Error(payload?.error || "Signed in, but workspace bootstrap failed. Try again.");
  }
    ...
    try {
      ...
      if (isSignIn) {
        ...
        try {
          ...
          setError(bootstrapError instanceof Error ? bootstrapError.message : "Signed in, but workspace bootstrap failed. Try again.");
          return;
        }
      ...
      if (data.session) {
        try {
          ...
          await supabase.auth.signOut();
          setError(bootstrapError instanceof Error ? bootstrapError.message : "Account created, but workspace bootstrap failed. Try signing in again.");
          return;
        }
