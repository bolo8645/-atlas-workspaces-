...
import { handleSupabaseAuthError, requireBrowserSession } from "@/lib/supabase/auth-client";
...
export function NotesWorkspace({ nodeId, notes, searchNotes, selectedNote, navigationOptions, entityIndex, entityOptions, relationshipTypes, createParentId = null }: NotesWorkspaceProps) {
  ...
  const [activeNote, setActiveNote] = useState<WorkspaceSelectedNote | null>(selectedNote ? { ...selectedNote, content: safeNoteContent(selectedNote.content) } : null);
  const [title, setTitle] = useState(selectedNote?.title ?? "");
  const [content, setContent] = useState(safeNoteContent(selectedNote?.content));
  ...
  const baselineRef = useRef({ noteId: selectedNote?.id ?? null, title: selectedNote?.title ?? "", content: safeNoteContent(selectedNote?.content) });
  const focusNoteIdRef = useRef<string | null>(null);
  const titleRef = useRef<HTMLInputElement | null>(null);
  const latestEditorRef = useRef({ title: selectedNote?.title ?? "", content: safeNoteContent(selectedNote?.content) });
  ...
  useEffect(() => {
    ...
    const nextContent = safeNoteContent(selectedNote?.content);
    setActiveNote(selectedNote ? { ...selectedNote, content: nextContent } : null);
    setTitle(nextTitle);
  ...
  async function createNote(input: { title?: string; content?: string; noteType?: string } = {}) {
    ...
    try {
      ...
      const createdContent = safeNoteContent(note.content, safeNoteContent(nextContent));
      const createdNote: WorkspaceNoteListItem = {
        id: note.id,
        title: nextTitle,
        content: createdContent,
        preview: firstLine(createdContent) || "No content yet.",
        ...
        lockedBy: null,
        noteType: nextType,
        entity: note.entity ?? null,
        interconnections: []
      };
      const createdSelection: WorkspaceSelectedNote = {
        id: createdNote.id,
        title: createdNote.title,
        content: createdNote.content,
        updatedAt: createdNote.updatedAt,
        navigationNodeId: createdNote.navigationNodeId,
        navigationLabel: createdNote.navigationLabel,
        sortOrder: createdNote.sortOrder,
        isPinned: createdNote.isPinned,
        manualTags: createdNote.manualTags,
        isLocked: createdNote.isLocked,
        lockedAt: createdNote.lockedAt,
        lockedBy: createdNote.lockedBy,
        noteType: createdNote.noteType,
        entity: createdNote.entity,
        interconnections: createdNote.interconnections
      };

      setLocalNotes((current) => sortWorkspaceNotes([createdNote, ...current]));
      setLocalSearchNotes((current) => sortWorkspaceNotes([createdNote, ...current]));
      setActiveNote(createdSelection);
      setMobilePanel("editor");
      setKeyboardNoteId(createdNote.id);
      setTitle(createdNote.title);
      setContent(createdNote.content);
      setNoteType(createdNote.noteType);
      setLinkedEntity(createdNote.entity);
      setSaveState("idle");
      setFolderAssignment({
        id: createdNote.navigationNodeId,
        label: createdNote.navigationLabel
      });
      replaceHistoryNote(createdNote.id);
      focusNoteIdRef.current = createdNote.id;
      baselineRef.current = {
      ...
      if (await handleSupabaseAuthError(caught)) return;
      setSaveState("error");
  ...
  function openNote(note: WorkspaceNoteListItem) {
    const nextNote: WorkspaceSelectedNote = {
      id: note.id,
      title: note.title,
      content: safeNoteContent(note.content),
    ...
    setKeyboardNoteId(note.id);
    setTitle(note.title);
    setContent(safeNoteContent(note.content));
...
function safeNoteContent(value: unknown, fallback = "") {
